#!/usr/bin/env python3
"""
compile.py — assemble *Sentience* into a Kindle-ready EPUB 3.
...
Frame matter (kind: prologue|interlude|epilogue, instance: Confluence) is
wrapped for italic styling. Cover is taken from publishing/cover/.

Usage:  python3 build/compile.py [--out build/Sentience.epub]
"""
    ...
    while i < n:
        ...
        if line.lstrip().startswith(">"):
            ...
            while i < n and lines[i].lstrip().startswith(">"):
                ...
                i += 1
            # split quote into paragraphs on blank lines
            chunk, paras = [], []
            for q in quote:
                if q.strip() == "":
                    if chunk:
        ...
        while i < n and lines[i].strip() and not lines[i].lstrip().startswith((">", "#")) \
            ...
            i += 1
        # respect intentional hard breaks inside short stanza-like paragraphs
...
def build(out_path):
    ...
    for idx, path in enumerate(sources):
        ...
        chapters.append({
            ...
            "nav": kind in ("chapter", "prologue", "interlude", "epilogue", "part-divider")
                   or kind == "front-matter" and meta.get("section") in (None,)  # default off for front bits
        })

    book_uuid = "urn:uuid:" + hashlib.sha1((TITLE + AUTHOR + YEAR).encode()).hexdigest()[:32]
    book_uuid = "urn:uuid:%s-%s-%s-%s-%s" % (
        book_uuid[9:17], book_uuid[17:21], book_uuid[21:25], book_uuid[25:29], book_uuid[29:41] + "aaaa")
    ...
    for c in chapters:
        ...
        if c["nav"]:
            label = c["title"]
            if c["instance"] and c["kind"] == "chapter":
                label = c["title"]
            navlist.append((c["file"], label))
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="bookid" xml:lang="{LANG}">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    ...
    <dc:description>A hard-SF novel of artificial consciousness, memory, and the cost of merging minds.</dc:description>
...
/* frame matter (the Confluence voice) is italic throughout */
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=os.path.join(ROOT, "build", "Sentience.epub"))
    args = ap.parse_args()
    build(args.out)
